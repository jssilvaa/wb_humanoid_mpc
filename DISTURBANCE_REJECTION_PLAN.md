...
- **Unified at C++20** (`ocs2_flags` = `-std=gnu++20 -Wno-invalid-partial-specialization -include cassert -O2`). OCS2's `std::result_of` (removed in C++20) was patched to `std::invoke_result` in 3 ocs2_core headers (portable; OCS2_CLANG_PATCHES.md #3). C++20 is REQUIRED because the closed-loop `CentroidalMpcMrtJointController` includes both OCS2 and `robot_model` headers (C++20 concepts/span via IDMapBase.h) in one TU, and uses `std::jthread`. (Initially tried a C++17/C++20 split; the MRT bridge forced unification.)
...
- **3 portable clang patches** in the `lib/ocs2_ros2` submodule (tracked in `OCS2_CLANG_PATCHES.md`, left uncommitted there): `MultidimensionalPenalty.cpp` (drop ctor template args), `SqpSolver.cpp` (`to_time_t` on `system_clock`), and the `result_of`→`invoke_result` set (3 ocs2_core headers).
- **`ocs2_ddp` = settings-only** (`DDP_Settings.cpp` + `search_strategy/StrategySettings.cpp` + `HessianCorrection.cpp`): the DDP solver proper (`DDP_DataCollector`) references OCS2 API removed in this version; we solve with SQP and only need `ddp::Settings`/`loadSettings` (which transitively need the 3 settings TUs).
- **Milestone** = `tools/ocs2SolveCheck.cpp` (OCS2 circular-kinematics OCP; PASS, violations ≈1e-32/1e-21).
- **de-ROS via `HUMANOID_MPC_NO_ROS2` macro** (the standalone build defines it; ament never does → that build stays byte-identical): guards `WalkingVelocityCommand.h` (msg-conversion fn), `GaitScheduleUpdater.h` (vestigial rclcpp), and `CentroidalMpcMrtJointController`'s optional `DummyObserver` viz (forward-declared). `humanoid_centroidal_mpc` now includes `mrt/` (links `robot::model`); the MRT controller solves the MPC→joint-torque tracking via inverse dynamics — reused by the B1 closed-loop harness.
